...
import json
import numpy as np
dataset_path = '/users/cx_xchen/DATASETS/'
img_anno = {
...
def process_images(gpu, data_chunk, save_path, if_parallel):
    ...
    for image_info in tqdm(data_chunk):
        if isinstance(image_info, dict):
            ...
            file_path = os.path.join(dataset_path,img_anno[data_name][0])
        else: 
...
if __name__ == '__main__':
    ...
    # File paths
    if img_anno[data_name][1] != '':
        json_file_path = os.path.join(dataset_path, img_anno[data_name][1])
        # Load data
        ...
        image_dir = os.path.join(dataset_path, img_anno[data_name][0])
        image_files = os.listdir(image_dir)
        data_chunks = np.array_split(image_files, num_gpus)
    # Create processes
    save_path = f'output/SAM_masks_pred/{sam_model}_{data_name}'
    ...
    if  not parallel:
        for gpu in range(num_gpus):
            p = mp.Process(target=process_images, args=(gpu, data_chunks[gpu],save_path, False))
            p.start()
            processes.append(p)
        for p in processes:
            p.join()
    elif num_gpus>1 and parallel:
        process_images(None, np.concatenate(data_chunks), save_path, if_parallel=True)
    else:
